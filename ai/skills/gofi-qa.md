...
5a. **Consultar o grafo antes de varrer o código.** Rode `gofi graph build --update` **primeiro**: o hook de pre-commit só reconstrói o grafo no commit e, numa cadeia `eng → qa`, o commit ainda não aconteceu — sem isto você auditaria um mapa sem a implementação. Depois `gofi_graph_index.json` → `gofi_graph_report.md` (as §"Ciclos de chamada" e §"Conexões inesperadas" são evidência direta de violação de camada) → `gofi graph explain <símbolo>` nos pontos auditados. Para **afirmar** que uma camada não chama outra, exija grafo `--deep` — em `fast` a ausência de aresta não é prova. **Nunca** abra `gofi_graph.json`. Protocolo: `.claude/knowledge/shared/graph-retrieval-protocol.md`
